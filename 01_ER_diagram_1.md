...
- 作成日時がない（問題点）
  > 登録日時のカラムはデフォルトで生成されます。特別な理由がない限りは記述しましょう。（レビュー）
- 更新日時がない（問題点）
  > 更新日時のカラムはデフォルトで生成されます。特別な理由がない限りは記述しましょう。（レビュー）
...
- 購入価格がない（問題点）
  > 算出は可能なので、保存はしなくてもよいです。ただ計算が面倒になるという点（テーブルをまたぐ）と購入者に表示するレシートのような役目を持っていますので、保存することになっています。（レビュー）
- 合計金額を保存するカラムがない（問題点）
  > 合計金額は保存しましょう。（レビュー）
- テーブル名と役割が合っていない（問題点）
...
- アーティスト名がある、ジャケット画像がある、シングル/アルバム名がある（問題点）
  > 商品テーブルと重複しているカラムがありますが、これは持たせる必要がありますか？（レビュー）
- 合計金額がある（問題点）
